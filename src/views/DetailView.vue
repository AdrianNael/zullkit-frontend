...
const route = useRoute(); //butuh dipanggil buat dinamis params
  ...
  try {
    const response = await axios.get(
      "https://zullkit-backend.belajarkoding.com/api/products?id=" +
        route.params.id
    ); //paramsm menggunakan id
    item.value = response.data.data; //manggil data kategori
  } catch (error) {
    console.log(eror);
  }
}
...
}); //data yang dijeda koma di split menjadi per array

onMounted(() => {
  window.scrollTo(0, 0);
  getProduct(); //jangan lupa buat ambil data
  userStore.fetchUser(); //ambil data paling baru memastikan data paling baru
});
</script>

<template>
  <!-- jika tidak ada data tidak dipanggil -->
  <main v-if="item">
    <div class="container p-2 mx-auto my-10 max-w-7xl">
      <div class="flex flex-row flex-wrap py-4">
        <main role="main" class="w-full px-4 pt-1 sm:w-2/3 md:w-2/3">
          ...
          <!-- binding data item ke props di gallery.vue -->
          <Gallery
          ...
          <section class="" id="orders">
            ...
            <!-- jika data ada HTML memakai seperti ini  -->
        ...
        <aside class="w-full px-4 sm:w-1/3 md:w-1/3">
          <div class="sticky top-0 w-full pt-4 md:mt-24">
            <div class="p-6 border rounded-2xl">
              ...
              <!-- jika data lebih dari 1 maka harus subscribe dulu -->
              <a
